...
public class ProcessorService {
    ...
    public static java.time.LocalDateTime getVersionDate() {
        return java.time.LocalDateTime.of(2019, 2, 21, 0, 0, 0);
    }
            ...
            if (img != null) {
                for (java.util.Map.Entry<String, byte[]> kp : img.entrySet()) {
                    if (!m_Images.containsKey(kp.getKey())) 
                        m_Images.put(kp.getKey(), ImageWrapper._new2818(kp.getKey(), kp.getValue()));
                }
            }
        } catch (Exception ex) {
        }
    ...
    public static ImageWrapper getImageById(String imageId) {
        if (imageId != null) {
            ImageWrapper res;
            com.pullenti.unisharp.Outargwrapper<ImageWrapper> wrapres2819 = new com.pullenti.unisharp.Outargwrapper<ImageWrapper>();
            boolean inoutres2820 = com.pullenti.unisharp.Utils.tryGetValue(m_Images, imageId, wrapres2819);
            res = wrapres2819.value;
            if (inoutres2820) 
                return res;
        }
        if (m_UnknownImage == null) 
            m_UnknownImage = ImageWrapper._new2818("unknown", com.pullenti.ner.core.internal.ResourceHelper.getBytes("unknown.png"));
    ...
    public static void addImage(String imageId, byte[] content) {
        if (imageId == null) 
            return;
        ImageWrapper wr = ImageWrapper._new2818(imageId, content);
