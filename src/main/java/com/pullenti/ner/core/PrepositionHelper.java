...
public class PrepositionHelper {
    ...
    public static PrepositionToken tryParse(com.pullenti.ner.Token t) {
        ...
        if (tok != null) 
            return PrepositionToken._new592(t, tok.getEndToken(), tok.termin.getCanonicText(), (com.pullenti.morph.MorphCase)tok.termin.tag);
    ...
    public static void initialize() {
        ...
        for (String s : new String[] {"близко от", "в виде", "в зависимости от", "в интересах", "в качестве", "в лице", "в отличие от", "в отношении", "в пандан", "в пользу", "в преддверии", "в продолжение", "в результате", "в роли", "в силу", "в случае", "в течение", "в целях", "в честь", "во имя", "вплоть до", "впредь до", "за вычетом", "за исключением", "за счет", "исходя из", "на благо", "на виду у", "на глазах у", "начиная с", "невзирая на", "недалеко от", "независимо от", "от имени", "от лица", "по линии", "по мере", "по поводу", "по причине", "по случаю", "поблизости от", "под видом", "под эгидой", "при помощи", "с ведома", "с помощью", "с точки зрения", "с целью"}) {
            m_Ontology.add(Termin._new593(s.toUpperCase(), com.pullenti.morph.MorphLang.RU, true, com.pullenti.morph.MorphCase.GENITIVE));
        }
        for (String s : new String[] {"вдоль по", "по направлению к", "применительно к", "смотря по", "судя по"}) {
            m_Ontology.add(Termin._new593(s.toUpperCase(), com.pullenti.morph.MorphLang.RU, true, com.pullenti.morph.MorphCase.DATIVE));
        }
        for (String s : new String[] {"несмотря на", "с прицелом на"}) {
            m_Ontology.add(Termin._new593(s.toUpperCase(), com.pullenti.morph.MorphLang.RU, true, com.pullenti.morph.MorphCase.ACCUSATIVE));
        }
        for (String s : new String[] {"во славу"}) {
            m_Ontology.add(Termin._new593(s.toUpperCase(), com.pullenti.morph.MorphLang.RU, true, (com.pullenti.morph.MorphCase.ooBitor(com.pullenti.morph.MorphCase.GENITIVE, com.pullenti.morph.MorphCase.DATIVE))));
        }
        for (String s : new String[] {"не считая"}) {
            m_Ontology.add(Termin._new593(s.toUpperCase(), com.pullenti.morph.MorphLang.RU, true, (com.pullenti.morph.MorphCase.ooBitor(com.pullenti.morph.MorphCase.GENITIVE, com.pullenti.morph.MorphCase.ACCUSATIVE))));
        }
        for (String s : new String[] {"в связи с", "в соответствии с", "вслед за", "лицом к лицу с", "наряду с", "по сравнению с", "рядом с", "следом за"}) {
            m_Ontology.add(Termin._new593(s.toUpperCase(), com.pullenti.morph.MorphLang.RU, true, com.pullenti.morph.MorphCase.INSTRUMENTAL));
        }
    }
