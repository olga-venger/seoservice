...
public class Referent {
    ...
    public int getIntValue(String attrName, int defValue) {
        ...
        if (com.pullenti.unisharp.Utils.isNullOrEmpty(str)) 
            return defValue;
        int res;
        com.pullenti.unisharp.Outargwrapper<Integer> wrapres2823 = new com.pullenti.unisharp.Outargwrapper<Integer>();
        boolean inoutres2824 = com.pullenti.unisharp.Utils.parseInteger(str, 0, null, wrapres2823);
        res = (wrapres2823.value != null ? wrapres2823.value : 0);
        if (!inoutres2824) 
            return defValue;
        return res;
    }
    ...
    public void addOccurenceOfRefTok(ReferentToken rt) {
        this.addOccurence(TextAnnotation._new719(rt.kit.getSofa(), rt.beginChar, rt.endChar, rt.referent));
    }
    ...
    public void addOccurence(TextAnnotation anno) {
        ...
        if (anno.getOccurenceOf() != this && anno.getOccurenceOf() != null) 
            anno = TextAnnotation._new2826(anno.beginChar, anno.endChar, anno.sofa);
        if (m_Occurrence == null) 
            m_Occurrence = new java.util.ArrayList<TextAnnotation>();
    ...
    public Referent clone() {
        ...
        for (Slot r : getSlots()) {
            Slot rr = Slot._new2827(r.getTypeName(), r.getValue(), r.getCount());
            rr.setOwner(res);
            res.getSlots().add(rr);
        }
        return res;
    ...
    public void deserialize(com.pullenti.unisharp.Stream stream, java.util.ArrayList<Referent> all, SourceOfAnalysis sofa) throws java.io.IOException {
        ...
        m_Occurrence = new java.util.ArrayList<TextAnnotation>();
        for (int i = 0; i < cou; i++) {
            TextAnnotation a = TextAnnotation._new2829(sofa, this);
            m_Occurrence.add(a);
