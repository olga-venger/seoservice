...
public class StreetDefineHelper {
    ...
    public static AddressItemToken tryParseStreet(java.util.ArrayList<StreetItemToken> sli, boolean extOntoRegim, boolean forMetro) {
        ...
        for (i = 0; i < sli.size(); i++) {
            if (i == 0 && sli.get(i).typ == StreetItemType.FIX && ((sli.size() == 1 || sli.get(1).typ != StreetItemType.NOUN))) 
                ...
                if (i == 0 && (i + 1) >= sli.size() && ((com.pullenti.unisharp.Utils.stringsEq(sli.get(i).termin.getCanonicText(), "ВОЕННЫЙ ГОРОДОК") || com.pullenti.unisharp.Utils.stringsEq(sli.get(i).termin.getCanonicText(), "ПРОМЗОНА")))) {
                    ...
                    return AddressItemToken._new87(AddressItemToken.ItemType.STREET, sli.get(0).getBeginToken(), sli.get(0).getEndToken(), stri0, true);
                }
        ...
        AddressItemToken res = AddressItemToken._new84(AddressItemToken.ItemType.STREET, rli.get(0).getBeginToken(), rli.get(0).getEndToken(), street);
        ...
        if (adj != null) {
            ...
            if (name != null && (((name.getMorph().getNumber().value()) & (com.pullenti.morph.MorphNumber.PLURAL.value()))) != (com.pullenti.morph.MorphNumber.UNDEFINED.value())) 
                s = com.pullenti.morph.Morphology.getWordform(adj.termin.getCanonicText(), com.pullenti.morph.MorphBaseInfo._new211(com.pullenti.morph.MorphClass.ADJECTIVE, com.pullenti.morph.MorphNumber.PLURAL));
            else if (adjGen != com.pullenti.morph.MorphGender.UNDEFINED) 
                s = com.pullenti.morph.Morphology.getWordform(adj.termin.getCanonicText(), com.pullenti.morph.MorphBaseInfo._new212(com.pullenti.morph.MorphClass.ADJECTIVE, adjGen));
            else if ((((adj.getMorph().getGender().value()) & (gen.value()))) == (com.pullenti.morph.MorphGender.UNDEFINED.value())) 
                s = com.pullenti.morph.Morphology.getWordform(adj.termin.getCanonicText(), com.pullenti.morph.MorphBaseInfo._new212(com.pullenti.morph.MorphClass.ADJECTIVE, adj.getMorph().getGender()));
            else 
                s = com.pullenti.morph.Morphology.getWordform(adj.termin.getCanonicText(), com.pullenti.morph.MorphBaseInfo._new212(com.pullenti.morph.MorphClass.ADJECTIVE, gen));
            adjStr = s;
    ...
    private static AddressItemToken tryDetectNonNoun(java.util.ArrayList<StreetItemToken> sli, boolean ontoRegim, boolean forMetro) {
        ...
        if (sli.size() == 1 && sli.get(0).typ == StreetItemType.NAME && ((ontoRegim || forMetro))) {
            ...
            AddressItemToken res0 = AddressItemToken._new87(AddressItemToken.ItemType.STREET, sli.get(0).getBeginToken(), sli.get(0).getEndToken(), street, true);
        ...
        return AddressItemToken._new87(AddressItemToken.ItemType.STREET, sli.get(0).getBeginToken(), sli.get(sli.size() - 1).getEndToken(), street, doubt);
    }
    ...
    private static AddressItemToken _tryParseFix(java.util.ArrayList<StreetItemToken> sits) {
        ...
        if (com.pullenti.unisharp.Utils.stringsEq(sits.get(0).termin.acronym, "МКАД")) {
            ...
            return AddressItemToken._new84(AddressItemToken.ItemType.STREET, t0, t1, str);
        }
        if (com.pullenti.ner.geo.internal.MiscLocationHelper.checkGeoObjectBefore(sits.get(0).getBeginToken()) || AddressItemToken.checkHouseAfter(sits.get(0).getEndToken().getNext(), false, true)) {
            ...
            return AddressItemToken._new84(AddressItemToken.ItemType.STREET, sits.get(0).getBeginToken(), sits.get(0).getEndToken(), str);
        }
        return null;
    }
