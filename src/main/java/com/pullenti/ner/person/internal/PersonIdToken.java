...
public class PersonIdToken extends com.pullenti.ner.MetaToken {
    ...
    private static PersonIdToken tryParse(com.pullenti.ner.Token t, PersonIdToken prev) {
        if (t.isValue("СВИДЕТЕЛЬСТВО", null)) {
            ...
            if (reg && ip) 
                return _new2444(t, tt1, Typs.KEYWORD, "СВИДЕТЕЛЬСТВО О ГОСУДАРСТВЕННОЙ РЕГИСТРАЦИИ ФИЗИЧЕСКОГО ЛИЦА В КАЧЕСТВЕ ИНДИВИДУАЛЬНОГО ПРЕДПРИНИМАТЕЛЯ");
        }
        ...
        if (tok != null) {
            ...
            PersonIdToken res = _new2444(tok.getBeginToken(), tok.getEndToken(), ty, tok.termin.getCanonicText());
            if (prev == null) {
                if (ty != Typs.KEYWORD) 
                    return null;
        ...
        if (t instanceof com.pullenti.ner.NumberToken) {
            StringBuilder tmp = new StringBuilder();
            PersonIdToken res = _new2446(t0, t, Typs.NUMBER);
        ...
        if ((prev != null && prev.typ == Typs.KEYWORD && (t instanceof com.pullenti.ner.TextToken)) && !t.chars.isAllLower() && t.chars.isLetter()) {
            ...
            if (rr != null && rr.typ == Typs.NUMBER) 
                return _new2444(t, t, Typs.SERIA, (((com.pullenti.ner.TextToken)com.pullenti.unisharp.Utils.cast(t, com.pullenti.ner.TextToken.class))).term);
        }
        if ((t != null && t.isValue("ОТ", "ВІД") && (t.getNext() instanceof com.pullenti.ner.ReferentToken)) && com.pullenti.unisharp.Utils.stringsEq(t.getNext().getReferent().getTypeName(), "DATE")) 
            return _new2447(t, t.getNext(), Typs.DATE, t.getNext().getReferent());
        return null;
    }
    ...
    public static void initialize() {
        ...
        t = com.pullenti.ner.core.Termin._new119("ПАСПОРТ", Typs.KEYWORD);
        t.addVariant("ПАССПОРТ", false);
        t.addVariant("ПАСПОРТНЫЕ ДАННЫЕ", false);
        t.addVariant("ВНУТРЕННИЙ ПАСПОРТ", false);
        m_Ontology.add(t);
        t = com.pullenti.ner.core.Termin._new119("ЗАГРАНИЧНЫЙ ПАСПОРТ", Typs.KEYWORD);
        t.addVariant("ЗАГРАНПАСПОРТ", false);
        t.addAbridge("ЗАГРАН. ПАСПОРТ");
        m_Ontology.add(t);
        t = com.pullenti.ner.core.Termin._new119("УДОСТОВЕРЕНИЕ ЛИЧНОСТИ", Typs.KEYWORD);
        t.addVariant("УДОСТОВЕРЕНИЕ ЛИЧНОСТИ ОФИЦЕРА", false);
        m_Ontology.add(t);
        t = com.pullenti.ner.core.Termin._new119("СВИДЕТЕЛЬСТВО О ГОСУДАРСТВЕННОЙ РЕГИСТРАЦИИ ФИЗИЧЕСКОГО ЛИЦА В КАЧЕСТВЕ ИНДИВИДУАЛЬНОГО ПРЕДПРИНИМАТЕЛЯ", Typs.KEYWORD);
        ...
        m_Ontology.add(t);
        t = com.pullenti.ner.core.Termin._new119("ВОДИТЕЛЬСКОЕ УДОСТОВЕРЕНИЕ", Typs.KEYWORD);
        m_Ontology.add(t);
        t = com.pullenti.ner.core.Termin._new119("ЛИЦЕНЗИЯ", Typs.KEYWORD);
        m_Ontology.add(t);
        t = com.pullenti.ner.core.Termin._new119("СЕРИЯ", Typs.SERIA);
        t.addAbridge("СЕР.");
        t.addVariant("СЕРИ", false);
        m_Ontology.add(t);
        t = com.pullenti.ner.core.Termin._new119("НОМЕР", Typs.NUMBER);
        t.addAbridge("НОМ.");
        t.addAbridge("Н-Р");
        t.addVariant("№", false);
        t.addVariant("N", false);
        m_Ontology.add(t);
        t = com.pullenti.ner.core.Termin._new119("ВЫДАТЬ", Typs.VIDAN);
        t.addVariant("ВЫДАВАТЬ", false);
        t.addVariant("ДАТА ВЫДАЧИ", false);
        t.addVariant("ДАТА РЕГИСТРАЦИИ", false);
        m_Ontology.add(t);
        t = com.pullenti.ner.core.Termin._new119("КОД ПОДРАЗДЕЛЕНИЯ", Typs.CODE);
        t.addAbridge("К/П");
        t.addAbridge("К.П.");
        m_Ontology.add(t);
        t = com.pullenti.ner.core.Termin._new119("РЕГИСТРАЦИЯ", Typs.ADDRESS);
    ...
    public static PersonIdToken _new2444(com.pullenti.ner.Token _arg1, com.pullenti.ner.Token _arg2, Typs _arg3, String _arg4) {
    ...
    public static PersonIdToken _new2446(com.pullenti.ner.Token _arg1, com.pullenti.ner.Token _arg2, Typs _arg3) {
        PersonIdToken res = new PersonIdToken(_arg1, _arg2);
        res.typ = _arg3;
        return res;
    }

    public static PersonIdToken _new2447(com.pullenti.ner.Token _arg1, com.pullenti.ner.Token _arg2, Typs _arg3, com.pullenti.ner.Referent _arg4) {
        PersonIdToken res = new PersonIdToken(_arg1, _arg2);
        res.typ = _arg3;
        res.referent = _arg4;
        return res;
