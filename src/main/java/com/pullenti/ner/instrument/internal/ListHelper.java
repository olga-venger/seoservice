...
public class ListHelper {
                        ...
                        if (((int)listBullet) != 0 && ch.getBeginToken().isChar(listBullet)) {
                            for (com.pullenti.ner.Token tt = ch.getBeginToken().getNext(); tt != null && (tt.endChar < ch.endChar); tt = tt.getNext()) {
                                if (tt.getPrevious().isChar('.') && com.pullenti.ner.core.MiscHelper.canBeStartOfSentence(tt)) {
                                    FragToken ch2 = FragToken._new1343(tt, ch.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.INDENTION, ch.number);
    ...
    private static int _analizeListItems(java.util.ArrayList<FragToken> chi, int ind) {
        ...
        if (res.getHasChanges() && res.getMultilineChangesValue() != null) {
            ...
            FragToken cit = FragToken._new1326(ci.getBeginToken(), ci.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.CITATION);
        ...
        for (LineToken li : lines) {
            ...
            FragToken ch = FragToken._new1343(li.getBeginToken(), li.getEndToken(), (li.isListItem ? com.pullenti.ner.instrument.InstrumentKind.LISTITEM : com.pullenti.ner.instrument.InstrumentKind.CONTENT), li.number);
            ...
            if (chli != null) {
                for (LineToken lt : chli) {
                    ch.children.add(FragToken._new1326(lt.getBeginToken(), lt.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.LISTITEM));
                }
                if (ch.beginChar < ch.children.get(0).beginChar) 
                    ch.children.add(0, FragToken._new1326(ch.getBeginToken(), ch.children.get(0).getBeginToken().getPrevious(), com.pullenti.ner.instrument.InstrumentKind.CONTENT));
            }
        }
        return ret;
    }
            ...
            if ((lines.get(i).typ == InstrToken1.Types.LINE && lines.get(i).numbers.size() == 0 && lines.get(i).getBeginToken().isValue("ПРИЛОЖЕНИЯ", "ДОДАТОК")) && lines.get(i + 1).numbers.size() > 0 && lines.get(i).getEndToken().isChar(':')) {
                ...
                for (++i; i < lines.size(); i++) {
                    if (lines.get(i).numbers.size() == 0) {{
                        ...
                        int nn;
                        com.pullenti.unisharp.Outargwrapper<Integer> wrapnn1531 = new com.pullenti.unisharp.Outargwrapper<Integer>();
                        boolean inoutres1532 = com.pullenti.unisharp.Utils.parseInteger(lines.get(i).numbers.get(0), 0, null, wrapnn1531);
                        nn = (wrapnn1531.value != null ? wrapnn1531.value : 0);
                        if (inoutres1532) 
                            num = nn + 1;
    ...
    public static void correctIndex(java.util.ArrayList<InstrToken1> lines) {
        ...
        if (index.size() == content.size() && index.size() > 2) {
            if ((indText * 10) < conText) {
                com.pullenti.unisharp.Utils.putArrayValue(lines, 0, InstrToken1._new1533(lines.get(0).getBeginToken(), lines.get(cInd - 1).getEndToken(), true, InstrToken1.Types.INDEX));
