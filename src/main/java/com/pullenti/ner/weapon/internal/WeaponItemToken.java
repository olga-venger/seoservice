...
public class WeaponItemToken extends com.pullenti.ner.MetaToken {
    ...
    private static WeaponItemToken _TryParse(com.pullenti.ner.Token t, WeaponItemToken prev, boolean afterConj, boolean attachHigh) {
            ...
            if (res.typ == Typs.MODEL) {
                ...
                if (tok.termin.tag2 instanceof java.util.ArrayList) {
                    ...
                    for (com.pullenti.ner.core.Termin to : li) {
                        WeaponItemToken wit = _new2731(t, tok.getEndToken(), (Typs)to.tag, to.getCanonicText(), tok.getBeginToken() == tok.getEndToken());
                        res.innerTokens.add(wit);
        ...
        if (nnn != null) {
            ...
            if (tit != null) {
                WeaponItemToken res = _new2732(t, tit.getEndToken(), Typs.NUMBER);
                res.value = tit.value;
        ...
        if (((t instanceof com.pullenti.ner.TextToken) && t.chars.isLetter() && t.chars.isAllUpper()) && (t.getLengthChar() < 4)) {
            if ((t.getNext() != null && ((t.getNext().isHiphen() || t.getNext().isChar('.'))) && (t.getNext().getWhitespacesAfterCount() < 2)) && (t.getNext().getNext() instanceof com.pullenti.ner.NumberToken)) {
                WeaponItemToken res = _new2733(t, t.getNext(), Typs.MODEL, true);
            ...
            if ((t.getNext() instanceof com.pullenti.ner.NumberToken) && !t.isWhitespaceAfter()) {
                WeaponItemToken res = _new2733(t, t, Typs.MODEL, true);
                res.value = (((com.pullenti.ner.TextToken)com.pullenti.unisharp.Utils.cast(t, com.pullenti.ner.TextToken.class))).term;
                res._correctModel();
                return res;
            }
            if (com.pullenti.unisharp.Utils.stringsEq((((com.pullenti.ner.TextToken)com.pullenti.unisharp.Utils.cast(t, com.pullenti.ner.TextToken.class))).term, "СП") && (t.getWhitespacesAfterCount() < 3) && (t.getNext() instanceof com.pullenti.ner.TextToken)) {
                ...
                if (pp != null && ((pp.typ == Typs.MODEL || pp.typ == Typs.BRAND))) {
                    WeaponItemToken res = _new2732(t, t, Typs.NOUN);
                    res.value = "ПИСТОЛЕТ";
        ...
        if (((t instanceof com.pullenti.ner.TextToken) && t.chars.isLetter() && !t.chars.isAllLower()) && t.getLengthChar() > 2) {
            ...
            if (prev != null && ((prev.typ == Typs.NOUN || prev.typ == Typs.MODEL || prev.typ == Typs.BRAND))) 
                ...
                ok = true;
            if (ok) {
                WeaponItemToken res = _new2733(t, t, Typs.NAME, true);
                res.value = (((com.pullenti.ner.TextToken)com.pullenti.unisharp.Utils.cast(t, com.pullenti.ner.TextToken.class))).term;
        ...
        if (t.isValue("МАРКА", null)) {
            ...
            if (com.pullenti.ner.core.BracketHelper.canBeStartOfSequence(t.getNext(), true, false)) {
                ...
                if (br != null) 
                    return _new2737(t, br.getEndToken(), Typs.BRAND, com.pullenti.ner.core.MiscHelper.getTextValue(br.getBeginToken(), br.getEndToken(), com.pullenti.ner.core.GetTextAttr.NO));
            }
            if ((t.getNext() instanceof com.pullenti.ner.TextToken) && t.getNext().getLengthChar() > 1 && !t.getNext().chars.isAllLower()) 
                return _new2737(t, t.getNext(), Typs.BRAND, (((com.pullenti.ner.TextToken)com.pullenti.unisharp.Utils.cast(t, com.pullenti.ner.TextToken.class))).term);
        }
        if (t.isValue("КАЛИБР", "КАЛІБР")) {
            com.pullenti.ner.Token tt1 = t.getNext();
            if (tt1 != null && ((tt1.isHiphen() || tt1.isChar(':')))) 
                tt1 = tt1.getNext();
            com.pullenti.ner.measure.internal.NumbersWithUnitToken num = com.pullenti.ner.measure.internal.NumbersWithUnitToken.tryParse(tt1, null, false, false, false);
            if (num != null && num.singleVal != null) 
                return _new2737(t, num.getEndToken(), Typs.CALIBER, com.pullenti.ner.core.NumberHelper.doubleToString(num.singleVal));
        }
        if (t instanceof com.pullenti.ner.NumberToken) {
            com.pullenti.ner.measure.internal.NumbersWithUnitToken num = com.pullenti.ner.measure.internal.NumbersWithUnitToken.tryParse(t, null, false, false, false);
            if (num != null && num.singleVal != null) {
                ...
                if (num.getEndToken().getNext() != null && num.getEndToken().getNext().isValue("КАЛИБР", "КАЛІБР")) 
                    return _new2737(t, num.getEndToken().getNext(), Typs.CALIBER, com.pullenti.ner.core.NumberHelper.doubleToString(num.singleVal));
            }
        }
        if (t.isValue("ПРОИЗВОДСТВО", "ВИРОБНИЦТВО")) {
            ...
            if (tt1 instanceof com.pullenti.ner.ReferentToken) {
                if ((tt1.getReferent() instanceof com.pullenti.ner._org.OrganizationReferent) || (tt1.getReferent() instanceof com.pullenti.ner.geo.GeoReferent)) 
                    return _new2742(t, tt1, Typs.DEVELOPER, tt1.getReferent());
            }
        }
        return null;
    }
    ...
    public static void initialize() {
        ...
        t = com.pullenti.ner.core.Termin._new119("ПИСТОЛЕТ", Typs.NOUN);
        m_Ontology.add(t);
        t = com.pullenti.ner.core.Termin._new119("РЕВОЛЬВЕР", Typs.NOUN);
        m_Ontology.add(t);
        t = com.pullenti.ner.core.Termin._new119("ВИНТОВКА", Typs.NOUN);
        m_Ontology.add(t);
        t = com.pullenti.ner.core.Termin._new119("РУЖЬЕ", Typs.NOUN);
        m_Ontology.add(t);
        t = com.pullenti.ner.core.Termin._new121("АВТОМАТ", Typs.NOUN, 1);
        m_Ontology.add(t);
        t = com.pullenti.ner.core.Termin._new121("КАРАБИН", Typs.NOUN, 1);
        m_Ontology.add(t);
        t = com.pullenti.ner.core.Termin._new143("ПИСТОЛЕТ-ПУЛЕМЕТ", "ПИСТОЛЕТ-ПУЛЕМЕТ", Typs.NOUN);
        m_Ontology.add(t);
        t = com.pullenti.ner.core.Termin._new119("ПУЛЕМЕТ", Typs.NOUN);
        m_Ontology.add(t);
        t = com.pullenti.ner.core.Termin._new119("ГРАНАТОМЕТ", Typs.NOUN);
        t.addVariant("СТРЕЛКОВО ГРАНАТОМЕТНЫЙ КОМПЛЕКС", false);
        m_Ontology.add(t);
        t = com.pullenti.ner.core.Termin._new119("ОГНЕМЕТ", Typs.NOUN);
        m_Ontology.add(t);
        t = com.pullenti.ner.core.Termin._new119("МИНОМЕТ", Typs.NOUN);
        m_Ontology.add(t);
        t = com.pullenti.ner.core.Termin._new2754("ПЕРЕНОСНОЙ ЗЕНИТНО РАКЕТНЫЙ КОМПЛЕКС", "ПЗРК", Typs.NOUN);
        m_Ontology.add(t);
        t = com.pullenti.ner.core.Termin._new2754("ПРОТИВОТАНКОВЫЙ РАКЕТНЫЙ КОМПЛЕКС", "ПТРК", Typs.NOUN);
        m_Ontology.add(t);
        t = com.pullenti.ner.core.Termin._new119("НАРУЧНИКИ", Typs.NOUN);
        m_Ontology.add(t);
        t = com.pullenti.ner.core.Termin._new119("БРОНЕЖИЛЕТ", Typs.NOUN);
        m_Ontology.add(t);
        t = com.pullenti.ner.core.Termin._new119("ГРАНАТА", Typs.NOUN);
        m_Ontology.add(t);
        t = com.pullenti.ner.core.Termin._new119("ЛИМОНКА", Typs.NOUN);
        m_Ontology.add(t);
        t = com.pullenti.ner.core.Termin._new119("НОЖ", Typs.NOUN);
        m_Ontology.add(t);
        for (String s : new String[] {"МАКАРОВ", "КАЛАШНИКОВ", "СИМОНОВ", "СТЕЧКИН", "ШМАЙСЕР", "МОСИН", "СЛОСТИН", "НАГАН", "МАКСИМ", "ДРАГУНОВ", "СЕРДЮКОВ", "ЯРЫГИН", "НИКОНОВ", "МАУЗЕР", "БРАУНИНГ", "КОЛЬТ", "ВИНЧЕСТЕР"}) {
            m_Ontology.add(com.pullenti.ner.core.Termin._new119(s, Typs.BRAND));
        }
        for (String s : new String[] {"УЗИ"}) {
            m_Ontology.add(com.pullenti.ner.core.Termin._new119(s, Typs.NAME));
        }
        t = com.pullenti.ner.core.Termin._new2763("ТУЛЬСКИЙ ТОКАРЕВА", "ТТ", "ТТ", Typs.MODEL);
        li = new java.util.ArrayList<com.pullenti.ner.core.Termin>();
        li.add(com.pullenti.ner.core.Termin._new119("ПИСТОЛЕТ", Typs.NOUN));
        li.add(com.pullenti.ner.core.Termin._new119("ТОКАРЕВ", Typs.BRAND));
        t.tag2 = li;
        m_Ontology.add(t);
        t = com.pullenti.ner.core.Termin._new2763("ПИСТОЛЕТ МАКАРОВА", "ПМ", "ПМ", Typs.MODEL);
        li = new java.util.ArrayList<com.pullenti.ner.core.Termin>();
        li.add(com.pullenti.ner.core.Termin._new119("ПИСТОЛЕТ", Typs.NOUN));
        li.add(com.pullenti.ner.core.Termin._new119("МАКАРОВ", Typs.BRAND));
        t.tag2 = li;
        m_Ontology.add(t);
        t = com.pullenti.ner.core.Termin._new2763("ПИСТОЛЕТ МАКАРОВА МОДЕРНИЗИРОВАННЫЙ", "ПММ", "ПММ", Typs.MODEL);
        li = new java.util.ArrayList<com.pullenti.ner.core.Termin>();
        li.add((tt = com.pullenti.ner.core.Termin._new119("ПИСТОЛЕТ", Typs.NOUN)));
        tt.addVariant("МОДЕРНИЗИРОВАННЫЙ ПИСТОЛЕТ", false);
        li.add(com.pullenti.ner.core.Termin._new119("МАКАРОВ", Typs.BRAND));
        t.tag2 = li;
        m_Ontology.add(t);
        t = com.pullenti.ner.core.Termin._new2763("АВТОМАТ КАЛАШНИКОВА", "АК", "АК", Typs.MODEL);
        li = new java.util.ArrayList<com.pullenti.ner.core.Termin>();
        li.add(com.pullenti.ner.core.Termin._new119("АВТОМАТ", Typs.NOUN));
        li.add(com.pullenti.ner.core.Termin._new119("КАЛАШНИКОВ", Typs.BRAND));
        t.tag2 = li;
        m_Ontology.add(t);
    }

    public static WeaponItemToken _new2731(com.pullenti.ner.Token _arg1, com.pullenti.ner.Token _arg2, Typs _arg3, String _arg4, boolean _arg5) {
    ...
    public static WeaponItemToken _new2732(com.pullenti.ner.Token _arg1, com.pullenti.ner.Token _arg2, Typs _arg3) {
        WeaponItemToken res = new WeaponItemToken(_arg1, _arg2);
        res.typ = _arg3;
        return res;
    }

    public static WeaponItemToken _new2733(com.pullenti.ner.Token _arg1, com.pullenti.ner.Token _arg2, Typs _arg3, boolean _arg4) {
        WeaponItemToken res = new WeaponItemToken(_arg1, _arg2);
        res.typ = _arg3;
        res.isDoubt = _arg4;
        return res;
    ...
    public static WeaponItemToken _new2737(com.pullenti.ner.Token _arg1, com.pullenti.ner.Token _arg2, Typs _arg3, String _arg4) {
        WeaponItemToken res = new WeaponItemToken(_arg1, _arg2);
        res.typ = _arg3;
        res.value = _arg4;
        return res;
    ...
    public static WeaponItemToken _new2742(com.pullenti.ner.Token _arg1, com.pullenti.ner.Token _arg2, Typs _arg3, com.pullenti.ner.Referent _arg4) {
        WeaponItemToken res = new WeaponItemToken(_arg1, _arg2);
        res.typ = _arg3;
        res.ref = _arg4;
        return res;
