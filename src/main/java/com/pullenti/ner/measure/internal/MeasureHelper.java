...
public class MeasureHelper {
    ...
    public static boolean tryParseDouble(String val, com.pullenti.unisharp.Outargwrapper<Double> f) {
        ...
        if (com.pullenti.unisharp.Utils.isNullOrEmpty(val)) 
            return false;
        boolean inoutres1589 = com.pullenti.unisharp.Utils.parseDouble(val.replace(',', '.'), null, f);
        if (val.indexOf(',') >= 0 && inoutres1589) 
            return true;
        boolean inoutres1588 = com.pullenti.unisharp.Utils.parseDouble(val, null, f);
        if (inoutres1588) 
            return true;
        return false;
    }
