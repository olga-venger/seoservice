...
public class DecreeHelper {
    ...
    public static java.time.LocalDateTime parseDateTime(String str) {
        ...
        try {
            ...
            int y;
            com.pullenti.unisharp.Outargwrapper<Integer> wrapy823 = new com.pullenti.unisharp.Outargwrapper<Integer>();
            boolean inoutres824 = com.pullenti.unisharp.Utils.parseInteger(prts[0], 0, null, wrapy823);
            y = (wrapy823.value != null ? wrapy823.value : 0);
            if (!inoutres824) 
                return null;
            int mon = 0;
            int day = 0;
            if (prts.length > 1) {
                com.pullenti.unisharp.Outargwrapper<Integer> wrapmon821 = new com.pullenti.unisharp.Outargwrapper<Integer>();
                boolean inoutres822 = com.pullenti.unisharp.Utils.parseInteger(prts[1], 0, null, wrapmon821);
                mon = (wrapmon821.value != null ? wrapmon821.value : 0);
                if (inoutres822) {
                    if (prts.length > 2) {
                        com.pullenti.unisharp.Outargwrapper<Integer> wrapday820 = new com.pullenti.unisharp.Outargwrapper<Integer>();
                        com.pullenti.unisharp.Utils.parseInteger(prts[2], 0, null, wrapday820);
                        day = (wrapday820.value != null ? wrapday820.value : 0);
                    }
                }
            }
            if (mon <= 0) 
    ...
    public static CanonicDecreeRefUri tryCreateCanonicDecreeRefUri(com.pullenti.ner.Token t) {
        ...
        if (dr != null) {
            if (dr.getKind() == com.pullenti.ner.decree.DecreeKind.PUBLISHER) 
                return null;
            res = CanonicDecreeRefUri._new825(t.kit.getSofa().getText(), dr, t.beginChar, t.endChar);
            ...
            if (rt.getBeginToken().isChar('(') && rt.getEndToken().isChar(')')) {
                res = CanonicDecreeRefUri._new825(t.kit.getSofa().getText(), dr, rt.getBeginToken().getNext().beginChar, rt.getEndToken().getPrevious().endChar);
                return res;
            }
        ...
        res = CanonicDecreeRefUri._new827(t.kit.getSofa().getText(), dpr, t.beginChar, t1.endChar, hasDiap);
    ...
    public static com.pullenti.ner.MetaToken checkNds(com.pullenti.ner.Token t, double nds, boolean ndsMustbeMoney) {
                ...
                if (ne != null && ne.exTyp == com.pullenti.ner.core.NumberExType.PERCENT) {
                    if (Math.abs(ne.getRealValue() - nds) > 0.0001) {
                        ...
                        if (hasNds) 
                            ok = true;
                        if (ok) 
                            return com.pullenti.ner.MetaToken._new828(tt, ne.getEndToken(), ("Размер НДС должен быть " + nds + "%, а не " + ne.getRealValue() + "%"));
                    }
        ...
        if (m1 == null) {
            if (ndsMustbeMoney) 
                return com.pullenti.ner.MetaToken._new828(ndsT0, ndsT1, "Размер НДС должен быть в денежном выражении");
            return null;
        }
        if (hasAll) 
            return null;
        ...
        if (delta > 0.011) {
            if ((delta < 1) && m1.getRest() == 0 && m.getRest() == 0) {
                ...
                com.pullenti.ner.money.MoneyReferent mr = com.pullenti.ner.money.MoneyReferent._new830(m1.getCurrency(), mustBe);
                return com.pullenti.ner.MetaToken._new828(t, tt, ("Размер НДС должен быть " + _outMoney(mr) + ", а не " + _outMoney(m1)));
            }
        }
        if (incl) 
            return null;
        ...
        if (m2 != null && hasAll) {
            ...
            if (delta > 0.01) {
                ...
                return com.pullenti.ner.MetaToken._new828(t, tt, err);
            }
        }
        return null;
    }
