...
public class DerivateGroup {
    ...
    public DerivateGroup createByPrefix(String pref, MorphLang lang) {
        DerivateGroup res = _new41(true, pref);
        for (DerivateWord w : words) {
            if (lang != null && !lang.isUndefined() && ((MorphLang.ooBitand(w.lang, lang))).isUndefined()) 
                continue;
            DerivateWord rw = DerivateWord._new42(res, pref + w.spelling, w.lang, w._class, w.aspect, w.reflexive, w.tense, w.voice, w.attrs);
            res.words.add(rw);
        }
        return res;
    }

    public static DerivateGroup _new41(boolean _arg1, String _arg2) {
